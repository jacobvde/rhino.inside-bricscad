...
namespace GH_BC
{
  public class BuildingElement : BakeComponent
  {
    ...
    { }
    public override Guid ComponentGuid => new Guid("B11776D0-6C3D-4323-B7AB-857009BB9236");
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
      ...
      Bricscad.Bim.BIMComposition bimComposition = null;
      Types.Composition composition = null;
      if (DA.GetData("Composition", ref composition))
      {
        var dummy = new Bricscad.Bim.BIMComposition(composition.Value);
        if (dummy.SaveComposition() == Bricscad.Bim.BimResStatus.Ok)
        {
  ...
  public class BuildingElement_OLD : BakeComponent
  {
    public BuildingElement_OLD()
      : base("Bake Building Element", "BBE", "Bake the Grasshopper geometry into the current BricsCAD drawing, while adding BIM data to it. The output of Bake Building Element is a reference to the baked building element with BIM data.", "BricsCAD", GhUI.BuildingElements)
    { }
